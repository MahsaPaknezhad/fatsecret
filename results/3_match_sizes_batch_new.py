...
def process_food_item(food_data):
    try:
        # Extract individual food items from the input
        foods = food_data.get('foods', [])
        all_results = []
        
        for food_item in foods:
            # Create input for single food item
            single_food_input = {
                'foods': [food_item],
            ...
            response = FoodMatchResponse(**response_json)
...
# Save results
with open('/home/ubuntu/projects/fatsecret/outputs/round3/3_match_sizes_batch_structured.json', 'w') as f:
